...
object Day19KMeansExample extends App {
  ...
  println(s"Testing k-means clustering with Scala version: ${util.Properties.versionNumberString}")
  val spark = SparkSession.builder().appName("test").master("local").getOrCreate()
  println(s"Session started on Spark version: ${spark.version}")

  //k-means clustering = dividing data in clusters

  val staticDataFrame = spark.read
    .option("header", "true")
    .option("inferSchema", "true") .csv("src/resources/retail-data/by-day/2010-12-*.csv") //so all of december 2010
  ...
  //splits are usually 80% of training and 20% of testing
  ...
  //going to perform some hyper-parameter tuning on the model because we do not want to repeat the
  ...
  transformedTraining.cache() //this turns on caching (/saving the working data set in memory)
  ...
  println("The results of our kmeans test:")
