...
object Day28StatiscticalFun extends App {

  println("Chapter 7: Statistical functions: variance, standard deviation, skewness, kurtosis, covariance, correlation!")
...
//Variance and Standard Deviation
  ...
  numDF.select(var_pop("nums"), var_samp("nums"),
    stddev_pop("nums"), stddev_samp("nums")).show()

  //in SQL
  spark.sql(
    """
      |SELECT var_pop(nums), var_samp(nums),
      |stddev_pop(nums), stddev_samp(nums)
      |FROM dfTable
      |""".stripMargin)
    .show()
