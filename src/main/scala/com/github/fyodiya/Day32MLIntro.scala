...
//WARNING
//Confusingly, there are similar data types that refer to ones that can be used in DataFrames and others
...
object Day32MLIntro extends App {
  ...
  val Array(train, test) = preparedDF.randomSplit(Array(0.7, 0.3)) //so roughly 70% to 30%
  ...
  //we will use test to see how well we did - it is crucial that none of these test data points were used in training
  test.describe().show()
