...
object Day19StreamingExample extends App {
  ...
  val staticDataFrame = spark.read
    ...
    .csv("src/resources/retail-data/by-day/*.csv") //in order for this to work Hadoop (via Winutils on Windows) should be installed
//      .load("src/resources/retail-data/by-day/*.csv") //notice the wildcard: we are loading everything!
  ...
  //checking if the Spark is streaming the data:
  println(s"The data is streaming: ${streamingDataFrame.isStreaming}")
