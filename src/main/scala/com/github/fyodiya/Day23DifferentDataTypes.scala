...
object Day23DifferentDataTypes extends App {
  ...
  println("Chapter 6. Working with Different Types of Data")
...
//  spark.conf.set("spark.sql.caseSensitive", true)
  ...
  df.select(col("description"), lit("five"), lit(5.0)).show(4) //all rows got these 3 new columns

  //There’s no equivalent function necessary in SQL,
  //that's why we can use the values directly:
  ...
  //selecting ALL rows which have InvoiceNo equal to 536365
